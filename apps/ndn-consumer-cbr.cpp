      ...
      .AddConstructor<ConsumerCbr>()
       ...
       .AddAttribute("Size", "Amount of data in megabytes to request, relying on PayloadSize "
                       "parameter (alternative to MaxSeq attribute)",
                        ...
                        MakeDoubleAccessor(&ConsumerCbr::GetMaxSize, &ConsumerCbr::SetMaxSize),
       ...
       .AddAttribute("PayloadSize",
                      "Average size of content object size (to calculate interest generation rate)",
                       UintegerValue(1040), MakeUintegerAccessor(&ConsumerCbr::GetPayloadSize,
                       &ConsumerCbr::SetPayloadSize),
...
uint32_t
ConsumerCbr::GetPayloadSize() const
{
  return m_payloadSize;
}

void
ConsumerCbr::SetPayloadSize(uint32_t payload)
{
  m_payloadSize = payload;
}
double
ConsumerCbr::GetMaxSize() const
{
  if (m_seqMax == 0)
    return -1.0;
    return m_maxSize;
...
void
ConsumerCbr::SetMaxSize(double size)
{
  m_maxSize = size;
  //Size of combined packet must be less than 1040 bytes, while the max MTU  for Ethernet is 1500 bytes.
  if ((m_maxSize < 0) && ( m_maxSize <  m_payloadSize) ) {
    m_seqMax = 0;
    return;
  }
