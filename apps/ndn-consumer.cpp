  ...
  , m_seqMax(0) // don't request anything
{
  ...
  newchunknumber=m_seq;// added by anselme
  uint32_t m_payloadSize=1040;
}
...
Consumer::SendPacket()
{
  ...
  if (seq == std::numeric_limits<uint32_t>::max()) {
    ...
    seq = m_seq++;
    newchunknumber++;
    chunk_number1=++newchunknumber;
    chunk_number2=++chunk_number1;
    chunk_number3+=++chunk_number2;
  }
//adding chunk number and range

  std::string mnewchunknumber= std::to_string(newchunknumber);
  std::string chunkrange1= std::to_string(chunk_number1);
  std::string chunkrange2= std::to_string(chunk_number2);

  Name dataName(m_interestName);
  dataName.append(mnewchunknumber + "/" + chunkrange1 + "/" + chunkrange2 + "/" );
