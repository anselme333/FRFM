...
//Added by Anselme
/*
uint32_t Interest::getChunkNumber() const
{
  if (!m_chunknumber.hasWire())
    const_cast<Interest*>(this)->setChunkNumber(chunk_number);

  if (m_chunknumber.value_size() == sizeof(uint32_t))
    return *reinterpret_cast<const uint32_t*>(m_chunknumber.value());
  else {
    // for compatibility reasons.  Should be removed eventually
    return readNonNegativeInteger(m_chunknumber);
  }
}

Interest& Interest::setChunkNumber( uint32_t chunk_number)
{
//int32_t chunknumber_counter=1;
chunk_number = chunknumber_counter;
if (m_wire.hasWire() && m_chunknumber.value_size() == sizeof(uint32_t)) {
  std::memcpy(const_cast<uint8_t*>(m_chunknumber.value()), &chunk_number, sizeof(chunk_number));
 }
 else {
	 m_chunknumber = makeBinaryBlock(tlv::Chunk_number,
                              reinterpret_cast<const uint8_t*>(&chunk_number),
                              sizeof(chunk_number));
  }
  return *this;
}
...
////////////////////////////////////////////////////////////////////////
  // added by Anselme
  //getChunkNumber();
 // totalLength += encoder.prependBlock(m_chunknumber);
 /////////////////////////////////////////////////////////////////
  // Selectors
  if (hasSelectors())
    {
...
Interest::wireDecode(const Block& wire)
{
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //Added by Anselme
  //m_chunknumber = m_wire.get(tlv::Chunk_number);
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // InterestLifetime
...
operator<<(std::ostream& os, const Interest& interest)
{
 ...
 // if (interest.hasChunkNumber()) {
   // os << delim << "ndn.Chunk_number=" << interest.getChunkNumber();
   //  delim = '&';
   //}
 //////////////////////////////////////////////////////////////////////////////////////////////////////
